...
public class SpacecraftInstrumentsQueryProvider {
   ...
   */
  private final EntityHelper<SpacecraftTemperatureOverTime> ehTemperature;
  private final EntityHelper<SpacecraftPressureOverTime> ehPressure;
  private final EntityHelper<SpacecraftLocationOverTime> ehLocation;
  private final EntityHelper<SpacecraftSpeedOverTime> ehSpeed;

  /**
   * Statements Against Astra.
   */
  private final PreparedStatement psInsertTemperatureReading;
  private final PreparedStatement psInsertPressureReading;
  private final PreparedStatement psInsertLocationReading;
  private final PreparedStatement psInsertSpeedReading;

  private final PreparedStatement psSelectTemperatureReading;
  private final PreparedStatement psSelectPressureReading;
  private final PreparedStatement psSelectLocationReading;
  private final PreparedStatement psSelectSpeedReading;

  /**
   * Constructor invoked by the DataStax driver based on Annotation {@link QueryProvider} set on class
   * {@link SpacecraftInstrumentsDao}.
   *
   * @param context       context to extrat dse session
   * @param ehTemperature entity helper to interact with bean {@link SpacecraftTemperatureOverTime}
   * @param ehPressure    entity helper to interact with bean {@link SpacecraftPressureOverTime}
   * @param ehLocation    entity helper to interact with bean {@link SpacecraftLocationOverTime}
   */
  ...
  public CompletionStage<Boolean> insertTemperatureReadingAsync(SpacecraftTemperatureOverTime[] readings) {
    ...
    Arrays
        .stream(readings)
        .forEach(read -> myBatch.addStatement(bind(psInsertTemperatureReading, read, ehTemperature)));
    return cqlSession.executeAsync(myBatch.build()).thenApply(rs -> rs.wasApplied());
  }
